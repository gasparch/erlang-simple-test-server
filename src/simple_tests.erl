...
-export([run/1, run/2, lookup_config/2, store/2, fetch/1]).
...
%%%
%%% server to keep value between cases run
%%% 
store_server() ->
	spawn(fun() -> init_store_server() end).

init_store_server() ->
	register(test_store_server, self()),
	run_store_server([]).

run_store_server(List) ->
	receive
	{get, Key, Pid} ->
		case lists:keysearch(Key, 1, List) of
		false -> Pid ! {test_store_server, undefined};
		{value, {_, Value}} -> 
			Pid ! {test_store_server, Value}
		end,
		run_store_server(List);
	{store, Key, Value, Pid} ->
		case lists:keysearch(Key, 1, List) of
		false -> 
			run_store_server([{Key, Value} | List ]);
		{value, _} -> 
				run_store_server(lists:keyreplace(Key, 1, List, {Key, Value}))
		end
	end.
			

store(Key, Value) ->
	test_store_server ! {store, Key, Value, self() }.

fetch(Key) ->
	test_store_server ! {get, Key, self() },

	receive
		{test_store_server, Value} -> Value
	after 1000 ->
		exit({error, test_store_server, timeouted})
	end.
